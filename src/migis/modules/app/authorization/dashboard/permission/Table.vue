...
export default {
  ...
  data() {
    return {
      fields: [
        ...
        {label: 'Teks', key: 'text',  thStyle: { width: "30%" }, thClass: 'text-left', tdClass: 'text-left' },
  ...
  created() {},
  mounted() {
    _socket.on('auth:permission:updated', async (data) => {
      await AuthorizationService.init()
      setTimeout(() => {
        this.$bvToast.toast(`Izin ${data.text} berhasil diupdate.`, {
          title: 'Authorization',
  ...
  beforeDestroy() {
    _socket.off('auth:permission:updated')
  },
}
</script>
